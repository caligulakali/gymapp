...
const EMPTY_FORM: FormState = { name: '', muscleGroup: 'chest', type: 'strength', unit: 'kg', notes: '', favourite: false };
...
export function ExercisePage({ repository, createId = makeId, onSelectExercise, onBack, onChanged }: ExercisePageProps) {
  ...
  const visibleExercises = useMemo(() => {
    ...
    return exercises
      ...
      .filter((exercise) => !normalizedQuery || `${exercise.name} ${getMuscleGroupLabel(exercise.muscleGroup)}`.toLocaleLowerCase('ru-RU').includes(normalizedQuery))
  ...
  function startEditing(exercise: Exercise): void {
    ...
    setForm({ name: exercise.name, muscleGroup: exercise.muscleGroup, type: exercise.type, unit: exercise.unit, notes: exercise.notes ?? '', favourite: exercise.favourite });
    setError(undefined);
    setShowDetails(Boolean(exercise.notes || exercise.favourite));
    setIsEditing(true);
  }
  ...
  if (isEditing) {
    return <section className="exercise-page exercise-editor" aria-labelledby="exercise-editor-title">
      ...
      <form className="exercise-form streamlined-form" onSubmit={handleSubmit}>
        <label className="prominent-field"><span>Название</span><input aria-label="Название" autoFocus placeholder="Например, жим гантелей" value={form.name} onChange={(event) => updateForm('name', event.target.value)} /></label>
        <section className="form-section" aria-labelledby="muscle-section-title"><div className="form-section-heading"><span className="form-step">01</span><div><h3 id="muscle-section-title">Что тренируем?</h3><p>Выбери основную мышечную группу</p></div></div><label className="muscle-group-field"><span className="field-label">Мышечная группа</span><span className="muscle-picker"><select className="muscle-group-native-select" aria-label="Мышечная группа" value={form.muscleGroup} onChange={(event) => updateForm('muscleGroup', event.target.value as MuscleGroup)} tabIndex={-1}>{MUSCLE_CATEGORIES.map((category) => <optgroup key={category.value} label={category.label}>{category.options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</optgroup>)}</select><button className={`muscle-picker-trigger${isMusclePickerOpen ? ' is-open' : ''}`} type="button" aria-label="Выбрать мышечную группу" aria-haspopup="listbox" aria-expanded={isMusclePickerOpen} onClick={() => setIsMusclePickerOpen((current) => !current)}><span className="muscle-picker-icon" aria-hidden="true">◇</span><span><strong>{getMuscleGroupLabel(form.muscleGroup)}</strong><small>Основная зона нагрузки</small></span><span className="muscle-picker-chevron" aria-hidden="true">⌄</span></button>{isMusclePickerOpen && <div className="muscle-picker-menu" role="listbox" aria-label="Группы мышц">{MUSCLE_CATEGORIES.map((category) => <div className="muscle-picker-category" key={category.value} role="group" aria-label={category.label}><div className="muscle-picker-category-heading"><span>{category.label}</span><small>{category.options.length} зоны</small></div><div className="muscle-picker-options">{category.options.map((option) => <button className={option.value === form.muscleGroup ? 'muscle-picker-option is-selected' : 'muscle-picker-option'} type="button" role="option" aria-selected={option.value === form.muscleGroup} key={option.value} onClick={() => { updateForm('muscleGroup', option.value); setIsMusclePickerOpen(false); }}><span>{option.label}</span>{option.value === form.muscleGroup && <span aria-hidden="true">✓</span>}</button>)}</div></div>)}</div>}</span></label></section>
        <section className="form-section" aria-labelledby="tracking-section-title"><div className="form-section-heading"><span className="form-step">02</span><div><h3 id="tracking-section-title">Как считать результат?</h3><p>Это определит поля во время тренировки</p></div></div><label className="type-field"><span className="field-label">Тип</span><select className="exercise-control-native-select" aria-label="Тип" value={form.type} onChange={(event) => updateForm('type', event.target.value as ExerciseType)} tabIndex={-1}>{TYPE_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><span className="type-picker" role="radiogroup" aria-label="Тип упражнения"><button className="type-picker-trigger" type="button" aria-label="Выбрать тип упражнения"><span className="control-icon" aria-hidden="true">○</span><span><strong>{TYPE_OPTIONS.find(([value]) => value === form.type)?.[1]}</strong><small>Текущий формат · килограммы</small></span></button><span className="type-picker-options">{TYPE_OPTIONS.map(([value, label]) => <button className={value === form.type ? 'type-picker-option is-selected' : 'type-picker-option'} type="button" role="radio" aria-checked={value === form.type} aria-label={label} key={value} onClick={() => updateForm('type', value)}><span className="type-option-dot" aria-hidden="true">{value === form.type ? '✓' : ''}</span><span>{label}</span></button>)}</span></span><select className="exercise-control-native-select" aria-label="Единица веса" value={form.unit} onChange={(event) => updateForm('unit', event.target.value as WeightUnit)} tabIndex={-1}><option value="kg">Килограммы (кг)</option></select></label></section>
        <button className={`details-toggle${showDetails ? ' is-open' : ''}`} type="button" aria-label="Дополнительно" aria-expanded={showDetails} onClick={() => setShowDetails((current) => !current)}><span aria-hidden="true">•••</span><span><strong>Дополнительно</strong><small>Заметки и избранное</small></span><span aria-hidden="true">⌄</span></button>
        {showDetails && <section className="form-section compact-options" aria-labelledby="details-section-title"><div className="form-section-heading"><span className="form-step">03</span><div><h3 id="details-section-title">Детали</h3><p>Необязательно, но удобно для себя</p></div></div><label className="notes-field exercise-notes-field"><span className="notes-heading"><span><strong>Заметки</strong><small>Техника, темп или настройки тренажёра</small></span><span>{form.notes?.length ?? 0}/240</span></span><textarea aria-label="Заметки" maxLength={240} placeholder="Можно оставить пустым" value={form.notes ?? ''} onChange={(event) => updateForm('notes', event.target.value)} /></label><label className={`favourite-toggle${form.favourite ? ' is-selected' : ''}`}><input className="favourite-input" type="checkbox" aria-label="Избранное" checked={form.favourite} onChange={(event) => updateForm('favourite', event.target.checked)} /><span className="favourite-star" aria-hidden="true">★</span><span><strong>Добавить в избранное</strong><small>Будет выше в списках</small></span><span className="favourite-check" aria-hidden="true">{form.favourite ? '✓' : ''}</span></label></section>}
