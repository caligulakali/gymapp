...
import { createExercise, EQUIPMENT_OPTIONS, getEquipmentLabel, MAX_REST_SECONDS, type ExerciseDraft } from '../../domain/exercise-service';
...
const EMPTY_FORM: FormState = { name: '', equipment: 'dumbbells', muscleGroup: 'chest', type: 'strength', unit: 'kg', notes: '', restSeconds: undefined, favourite: false };
...
export function ExercisePage({ repository, createId = makeId, onSelectExercise, onBack, onChanged }: ExercisePageProps) {
  ...
  function startEditing(exercise: Exercise): void {
    ...
    setForm({ name: exercise.name, equipment: exercise.equipment ?? 'other', muscleGroup: exercise.muscleGroup, type: exercise.type, unit: exercise.unit, notes: exercise.notes ?? '', restSeconds: exercise.restSeconds, favourite: exercise.favourite });
    setError(undefined);
    setIsEditing(true);
  }
  ...
  if (isEditing) {
    return <section className="exercise-page exercise-editor" aria-labelledby="exercise-editor-title">
      ...
      <form className="exercise-form streamlined-form simple-exercise-form" onSubmit={handleSubmit}>
        ...
        <section className="simple-choice-field exercise-details" aria-labelledby="exercise-details-title">
          <h3 id="exercise-details-title">5 · Детали</h3>
          <div className={`rest-setting${typeof form.restSeconds === 'number' ? ' is-enabled' : ''}`}>
            <label className="rest-setting-toggle"><input type="checkbox" aria-label="Использовать таймер отдыха" checked={typeof form.restSeconds === 'number'} onChange={(event) => updateForm('restSeconds', event.target.checked ? 90 : undefined)} /><span className="rest-setting-icon" aria-hidden="true">◷</span><span><strong>Таймер отдыха</strong><small>Запускай во время тренировки</small></span><span className="rest-setting-check" aria-hidden="true">{typeof form.restSeconds === 'number' ? '✓' : ''}</span></label>
            {typeof form.restSeconds === 'number' && <div className="rest-setting-controls"><label><span>Время отдыха</span><span className="rest-seconds-input"><input type="number" inputMode="numeric" min="1" max={MAX_REST_SECONDS} step="1" aria-label="Время отдыха" value={form.restSeconds} onChange={(event) => updateForm('restSeconds', event.target.value ? Number(event.target.value) : 90)} /><small>сек</small></span></label><div className="rest-presets" aria-label="Быстрый выбор времени отдыха">{[60, 90, 120].map((seconds) => <button className={form.restSeconds === seconds ? 'is-selected' : ''} type="button" key={seconds} onClick={() => updateForm('restSeconds', seconds)}>{seconds} с</button>)}</div></div>}
          </div>
          <label className="notes-field exercise-notes-field"><span>Заметки</span><textarea aria-label="Заметки" maxLength={240} placeholder="Техника, темп или настройки тренажёра" value={form.notes ?? ''} onChange={(event) => updateForm('notes', event.target.value)} /></label>
